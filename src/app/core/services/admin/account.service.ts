...
export class AccountService {
  ...
  getTotalAccountsByRole(role: string): Observable<{ totalAccount: number }> {
    return this.http.get<{ totalAccount: number }>(
      `${this.accountApiUrl}/role/total/${role}`
    );
  }
  ...
  getAccountById(accountId: number): Observable<AccountRequest> {
    return this.http
      .get<AccountRequest>(`${this.accountApiUrl}/${accountId}`)
      .pipe(
        catchError(this.handleError) // Bắt lỗi
      );
  }
  ...
  // Hàm lấy tài khoản theo roleName với pagination
  getAccountsByRole(
    roleName: string,
    page: number,
    size: number
    ...
    return this.http.get<{
      ...
      totalElements: number;
      totalPages: number;
    }>(`${this.accountApiUrl}/role/${roleName}?page=${page}&size=${size}`);
  }

  // Hàm cập nhật trạng thái tài khoản
  updateAccountStatus(accountId: number): Observable<AccountResponse> {
    return this.http
      .put<AccountResponse>(`${this.accountApiUrl}/${accountId}/status`, {})
      .pipe(
        catchError(this.handleError) // Bắt lỗi
      );
  }
